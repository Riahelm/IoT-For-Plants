...
#define AOUT_PIN 36
#define YELLOW_RGB_PIN 32
#define RED_RGB_PIN 25
#define GREEN_RGB_PIN 26
#define BLUE_RGB_PIN 27
...
void doTheThing(){
  ...
  int minVal= 1000;
  int maxVal= 2000;
  int normalVal = 100 - map(value, minVal, maxVal, 0, 100);
  int target = 40;
  int delta = 5;
  if(normalVal < target - delta){
    digitalWrite(RED_RGB_PIN, HIGH);
    digitalWrite(GREEN_RGB_PIN, LOW);
    digitalWrite(BLUE_RGB_PIN, LOW);
  }else if(normalVal < target + delta){
